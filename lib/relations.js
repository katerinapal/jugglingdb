...
var ModelBinding = _model.AbstractClass;


ModelBinding.relationNameFor = function relationNameFor(foreignKey) {
    var _this = this;

    var found = void 0;
    Object.keys(this.relations).forEach(function (rel) {
        if (_this.relations[rel].type === 'belongsTo' && _this.relations[rel].keyFrom === foreignKey) {
            found = rel;
        }
    });
    return found;
 ...
 */
ModelBinding.hasMany = function hasMany(anotherClass, params) {
    var _this2 = this;
    ...
    if (typeof anotherClass === 'string') {
        ...
        if (params.model) {
            ...
            var anotherClassName = _inflection2.default.singularize(anotherClass).toLowerCase();
            Object.keys(this.schema.models).forEach(function (name) {
                if (name.toLowerCase() === anotherClassName) {
                    anotherClass = _this2.schema.models[name];
                }
            });
        }
    }
    var methodName = params.as || _inflection2.default.camelize(_inflection2.default.pluralize(anotherClass.modelName), true);
    var fk = params.foreignKey || _inflection2.default.camelize(thisClassName + '_id', true);
    ...
    if (params.through) {
        ...
        fk2 = _inflection2.default.camelize(anotherClass.modelName + '_id', true);
        scopeMethods.create = function create(data, done) {
            ...
            return anotherClass.create(data).then(function (ac) {
                ...
                return params.through.create(d).then(function () {
                    return ac;
                }).catch(function (e) {
                    return ac.destroy().then(function () {
                        throw e;
                    });
                });
            }).then(function (ac) {
        ...
        scopeMethods.add = function (acInst, data, done) {
            ...
            return params.through.findOrCreate({ where: query }, data).then(function (through) {
                if (typeof done === 'function') {
                    done(null, through);
                } else {
                    return through;
    ...
    (0, _scope.defineScope)(this.prototype, params.through || anotherClass, methodName, function () {
        var filter = {};
        filter.where = {};
        filter.where[fk] = this.id;
        if (params.through) {
            filter.collect = _inflection2.default.camelize(anotherClass.modelName, true);
    ...
    function destroy(id, cb) {
        ...
        return anotherClass.find(id).then(function (inst) {
            ...
            if (inst[fk] && inst[fk].toString() === id_.toString()) {
                return inst.destroy().then(function () {
                    if (typeof cb === 'function') {
                        cb();
                    }
                });
 ...
 */
ModelBinding.belongsTo = function (anotherClass, params) {
    var _this3 = this;
    ...
    var methodName = params.as || _inflection2.default.camelize(anotherClass.modelName, true);
 ...
 */
ModelBinding.hasAndBelongsToMany = function hasAndBelongsToMany(anotherClass, params) {
    params = params || {};
    ...
    if (typeof anotherClass === 'string') {
        ...
        if (params.model) {
            ...
            anotherClass = lookupModel(_inflection2.default.singularize(anotherClass)) || anotherClass;
        }
        if (typeof anotherClass === 'string') {
