...
module.exports = function init(root) {
    ...
    if (!railway.parent) {
        ...
        if (config.driver && config.driver.match(/^\./)) {
            config.driver = _path2.default.join(app.root, config.driver);
        }

        schema = new _schema.Schema(config && config.driver || 'memory', config);
        schema.log = log;
        if (!schema.adapter) {
    ...
    function loadSchema(schema, railway, app, models) {
        ...
        if (schemaFile) {
            var code = _fs2.default.readFileSync(schemaFile).toString();
    ...
    function prepareContext(models, railway, app, defSchema, done) {
        ...
        ctx.schema = function () {
            ...
            schema = new _schema.Schema(name || opts.driver || 'memory', opts);
            railway.orm._schemas.push(schema);
            wait += 1;
            ctx.gotSchema = true;
            schema.on('log', log);
         ...
         */
        if (_schema.Schema.types && Object.keys(_schema.Schema.types).length) {
            Object.keys(_schema.Schema.types).forEach(function (typeName) {
                ctx[typeName] = _schema.Schema.types[typeName];
            });
        }
