...
var AbstractClassBinding = _model2.AbstractClass;
...
AbstractClassBinding.include = function (objects, include, cb) {
...
function processIncludeItem(cls, objs, include, keyVals, objsByKeys) {
    ...
    var inValues = [];
    for (var j = 0; j < keyVals[relation.keyFrom].length; j += 1) {
    ...
    function done(err, objsIncluded, cb) {
        var objectsFrom = void 0;

        for (var i = 0; i < objsIncluded.length; i += 1) {
            ...
            for (var _j = 0; _j < objectsFrom.length; _j += 1) {
                if (!objectsFrom[_j].__cachedRelations) {
                    objectsFrom[_j].__cachedRelations = {};
                }

                if (relation.multiple) {
                    if (!objectsFrom[_j].__cachedRelations[relationName]) {
                        objectsFrom[_j].__cachedRelations[relationName] = [];
                    }

                    if (_through) {
                        objectsFrom[_j].__cachedRelations[relationName].push(objsIncluded[i].__cachedRelations[_through]);
                    } else {
                        objectsFrom[_j].__cachedRelations[relationName].push(objsIncluded[i]);
                    }
                } else if (_through) {
                    objectsFrom[_j].__cachedRelations[relationName] = objsIncluded[i].__cachedRelations[_through];
                } else {
                    objectsFrom[_j].__cachedRelations[relationName] = objsIncluded[i];
                }
            }
        }
        ...
        Object.keys(keysToBeProcessed).forEach(function (key) {
            ...
            for (var k = 0; k < objectsFrom.length; k += 1) {
    ...
    if (keyVals[relation.keyFrom].length > 0) {
        ...
        if (relation.modelThrough) {
            ...
            _through = _inflection2.default.camelize(relation.modelTo.modelName, true);
        } else {
            req['where'][relation.keyTo] = { inq: inValues };
